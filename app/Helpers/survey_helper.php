...
if (!function_exists('getDisplayTextForAnswer')) {
    ...
    function getDisplayTextForAnswer($question, $storedAnswer)
    {
        ...
        switch ($type) {
            ...
            case 'videoImage':
                return displayVideoImageAnswer($storedAnswer);
...
if (!function_exists('calculate_question_score')) {
    ...
    function calculate_question_score($question, $answer)
    {
        ...
        switch ($type) {
            ...
            case 'videoImage':
                // No scoring for videoImage type
                return 0;
...
if (!function_exists('displayVideoImageAnswer')) {
    ...
    function displayVideoImageAnswer($answer)
    {
        ...
        $html = '<div class="grid grid-cols-2 md:grid-cols-3 gap-4 mt-3 video-image-container">';

        foreach ($answerData['files'] as $file) {
            $html .= '<div class="relative bg-gray-50 rounded-lg p-3 border video-image-item">';

            if (str_contains($file['mime_type'], 'image')) {
                // Display image with lightbox functionality
                $html .= '<a href="' . $file['url'] . '" target="_blank" class="block video-image-link">';
                $html .= '<img src="' . $file['url'] . '" alt="' . htmlspecialchars($file['original_name']) . '" ';
                $html .= 'class="w-full h-32 object-cover rounded-lg hover:opacity-80 transition-opacity video-image-media">';
                $html .= '</a>';
            } else if (str_contains($file['mime_type'], 'video')) {
                // Display video player
                $html .= '<video controls class="w-full h-32 rounded-lg bg-black video-image-media">';
